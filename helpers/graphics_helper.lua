...
function helper.add_flat_poly_angle(mesh, center, sides, color, radius,mul, startangle)
  if sides > 2 then
    local x = center[1]
    local y = center[2]
    local z = center[3]
    ...
    local colors = {}
    for i = 1, sides do
      local angle = (math.pi * mul * i)/sides + startangle
...
function helper.add_stars(mesh,center,width,height,depth,amount, colors)
  local x = center[1]
  local y = center[2]
  local z = center[3]

  for i = 1, amount do
    local x2 = fmath.random_int(x-width,width)
    local y2 = fmath.random_int(y-height,height)
    local z2 = fmath.random_int(z-depth,depth)

    helper.add_dot(mesh,{x2,y2,z2},colors)
  end
end
...
function helper.add_rectangle(mesh,center,width,height,color)
  local x,y,z = center[1],center[2],center[3]

  local a = {x-width/2,y-height/2,z}
  local b = {x-width/2,y+height/2,z}
  local c = {x+width/2,y+height/2,z}
  local d = {x+width/2,y-height/2,z}
